...
export default function SwipeScreen() {
  ...
  useEffect(() => {
    ...
    if (currentMacroIndex < macroOrder.length) {
      ...
      // ⚡ Bolt: Optimize dataset filtering in swipe screen by avoiding array mapping and nested O(N) array includes
      let itemsForPart = clothes.filter(c => c.part === currentPart);
      if (selectedCollectionId !== 'all') {
        const col = collections.find(c => c.id === selectedCollectionId);
        if (col) {
          const collectionSet = new Set(col.itemIds);
          itemsForPart = itemsForPart.filter(c => collectionSet.has(c.id));
        } else {
          itemsForPart = [];
        }
      }
