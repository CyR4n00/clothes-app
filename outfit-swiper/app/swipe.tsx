  ...
  useEffect(() => {
    ...
    if (currentMacroIndex < macroOrder.length) {
      ...
      if (selectedCollectionId === 'all') {
        ...
        // Optimization: Pre-compute allowed item IDs into a Set for O(1) lookup
        // instead of O(N) Array.includes() inside the filter loop.
        const colItemSet = new Set(col ? col.itemIds : []);
        itemsForPart = clothes.filter(c =>
          c.part === currentPart && colItemSet.has(c.id)
        );
      }
