...
export default function SwipeScreen() {
  ...
  useEffect(() => {
    ...
    if (currentMacroIndex < macroOrder.length) {
      ...
      // ⚡ Bolt: Replace O(N) array includes with O(1) Set lookup inside filter loop
      let collectionItemIdsSet: Set<string> | null = null;
      if (selectedCollectionId !== 'all') {
        const col = collections.find(c => c.id === selectedCollectionId);
        if (col) collectionItemIdsSet = new Set(col.itemIds);
        else collectionItemIdsSet = new Set();
      }

      const itemsForPart = clothes.filter(c =>
        c.part === currentPart && (selectedCollectionId === 'all' || (collectionItemIdsSet && collectionItemIdsSet.has(c.id)))
      );
